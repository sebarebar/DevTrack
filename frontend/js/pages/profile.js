...
export async function renderOwnProfile(params, app) {
  const content = renderShell(app, {
    ...
    hero: {
      eyebrow: 'Dev Card',
      title: 'Your <em>developer</em> card.',
      desc: 'This is how the world sees you. Share your public profile using the link below.',
    },
  });

  content.innerHTML = '<div class="page-loading">Loading profile...</div>';
  ...
  try {
    ...
    content.innerHTML =
      ...
      `<p class="section-desc" style="margin-top:1.5rem;">
        Public link:
        <a class="proj-link" href="/profile/${encodeURIComponent(me.username)}" data-route>
          ${escapeHtml(shareUrl)}
        </a>
      </p>`;
...
export async function renderPublicProfile(params, app) {
  ...
  app.innerHTML = `
    <header class="topbar">
      ...
      <a href="/dashboard" data-route class="topbar-logo">
        dev<span>track</span>
      </a>

      <nav class="topbar-nav">
        <a href="/login" data-route class="nav-tab">
          Login
        </a>
      </nav>
...
function devCardHtml({ user, skills, projects, badges }) {
  ...
  const skillPills = skills.length
    ...
    ? skills
        .map((s) => {
          ...
          return `<span 
            class="level-chip" 
            style="text-transform:none;color:var(--text);background:var(--surface);"
          >
            ${escapeHtml(s.name)} · ${lvl.label}
          </span>`;

        })
        .join(' ')

    : '<p class="section-desc">No skills yet.</p>';
  ...
  const badgeItems = badges.length
    ...
    : '<p class="section-desc">No badges earned yet.</p>';
    ...
    <div class="devcard">
      ...
      <div class="devcard-head">
        ...
        <div>
          ...
          <p class="devcard-name">
            ${escapeHtml(user.full_name || user.username)}
          </p>


          <p class="devcard-user">
            @${escapeHtml(user.username)}
            ${
              user.github_username
                ? ` · github.com/${escapeHtml(user.github_username)}`
                : ''
            }
          </p>
          ...
          <span class="level-chip">
            ${escapeHtml(user.current_level)} · ${user.total_points} pts
          </span>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <p class="stat-value green">${user.current_streak}</p>
        </div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 1.5rem;">
        ${skillPills}
      </div>
      ...
      <div class="projects-grid" style="margin:10px 0 1.5rem;">
        ${projectItems}
      </div>
      ...
      <div class="badges-grid" style="margin-top:10px;">
        ${badgeItems}
      </div>
