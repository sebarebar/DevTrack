...
export async function renderSkills(params, app) {
  const content = renderShell(app, {
    ...
    hero: {
      eyebrow: 'Tecnologías',
      title: 'Tu stack como <em>developer</em>, en un solo lugar.',
      desc: 'Registra las tecnologías que dominas, organizadas por categoría, y mide tu progreso.',
    },
  });

  content.innerHTML = '<div class="page-loading">Cargando skills...</div>';
  ...
  content.innerHTML = `
    <div class="form-card">
      ...
      <form id="skill-form" class="form-row">
        <div class="f-field f-grow">
          ...
          <input class="f-input" name="name" type="text" placeholder="Ej. TypeScript" required />
        </div>
    ...
    <div class="filter-bar">
      <div class="pills-group" id="skill-pills">
        <button class="fpill active" data-cat="all">Todas</button>
        ${state.categories
          .map((c) => `<button class="fpill" data-cat="${escapeHtml(c.name)}">${escapeHtml(c.name)}</button>`)
          .join('')}
      </div>
  ...
  document.getElementById('skill-form').addEventListener('submit', onAdd);
  document.getElementById('skill-pills').addEventListener('click', (e) => {
    const pill = e.target.closest('.fpill');
    if (!pill) return;
    document.querySelectorAll('#skill-pills .fpill').forEach((p) => p.classList.remove('active'));
...
function paint() {
  ...
  const visible = state.skills.filter((s) => {
    ...
    const matchSearch = !state.search || s.name.toLowerCase().includes(state.search);
  ...
  if (visible.length === 0) {
    grid.innerHTML = `
      <div class="empty-state">
        ...
        <p>No tienes skills que coincidan con este filtro.</p>
      </div>`;
    return;
  }
    ...
    .map((s) => {
      ...
      return `
        <div class="skill-card">
          <button class="del-btn" data-del="${s.id}" title="Eliminar">
            <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div class="sc-logo" style="background:${categoryGradient(s.category)}">${initial}</div>
          <p class="sc-name">${escapeHtml(s.name)}</p>
          <p class="sc-cat ${slug}">${escapeHtml(s.category || 'Otros')}</p>
          <div class="sc-footer">
            <div class="sc-stats">
              <span class="sc-level">${lvl.label}</span>
              <button class="sc-edit" data-edit="${s.id}">Cambiar nivel</button>
            </div>
            <div class="sc-bar-track"><div class="sc-bar-fill" style="width:${lvl.width}%"></div></div>
          </div>
        </div>`;
    })
    .join('');
  ...
  grid.querySelectorAll('[data-edit]').forEach((btn) =>
    btn.addEventListener('click', () => onEditLevel(btn.dataset.edit))
  );
}
...
async function onEditLevel(id) {
  ...
  const options = LEVELS.map((l) => `${l.order}=${l.label}`).join(', ');
  const input = prompt(`Nuevo nivel para ${skill.name} (${options}):`);
  if (!input) return;
  const chosen = LEVELS.find((l) => String(l.order) === input.trim());
  if (!chosen) {
    toast('Nivel inválido.', 'error');
    return;
  }
  try {
    await api.put(`/skills/${id}`, { level: chosen.value });
