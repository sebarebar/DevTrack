...
export function renderAuth(mode) {
  return (params, app) => {
    ...
    app.innerHTML = `
      <div class="auth-wrap">
        <aside class="auth-aside grid-bg">
          ...
          <div class="auth-hero">
            <h1>Tu progreso<br/>como <span>developer</span>,<br/>visualizado.</h1>
            <p>Registra skills, proyectos y horas de estudio. Consigue badges. Mira cómo creces.</p>
            <div class="auth-progress">
              <div style="display:flex;justify-content:space-between;font-size:12px;color:#94a3b8;margin-bottom:8px;">
                <span>JavaScript · Avanzado</span><span>72%</span>
              </div>
        ...
        <section class="auth-section">
          <div class="auth-card">
            <div class="auth-toggle">
              ...
              <button data-mode="register">Registro</button>
            </div>
      ...
      container.innerHTML = `
        <div style="margin-bottom:1.5rem;">
          ...
          <p class="fade-in-up" style="font-size:14px;color:#64748b;margin-top:4px;animation-delay:60ms;">${
            isLogin ? 'Ingresa tus datos para continuar.' : 'Empieza a registrar tu progreso hoy.'
          }</p>
        </div>
        <form id="auth-form" novalidate style="display:flex;flex-direction:column;gap:1.25rem;">
          ${isLogin ? '' : field('full_name', 'Nombre completo', 'text', 'Tu nombre', 1)}
          ${isLogin ? '' : field('username', 'Usuario', 'text', 'tu_usuario', 2)}
          ${field('email', 'Email', 'email', 'tucorreo@ejemplo.com', isLogin ? 1 : 3)}
          ${field('password', 'Contraseña', 'password', 'Mínimo 8 caracteres', isLogin ? 2 : 4)}
          ${isLogin ? '' : field('confirm', 'Confirmar contraseña', 'password', 'Repite tu contraseña', 5)}
          <p id="form-error" class="form-error hidden"></p>
          <button type="submit" id="submit-btn" class="btn-primary fade-in-up" style="width:100%;animation-delay:300ms;">
            <span id="btn-label">${isLogin ? 'Iniciar sesión' : 'Crear cuenta'}</span>
          </button>
        </form>
      `;
    ...
    const validate = (values) => {
      const errors = {};
      const isLogin = current === 'login';
      if (!isLogin && !values.full_name) errors.full_name = 'El nombre es obligatorio';
      if (!isLogin && !values.username) errors.username = 'El usuario es obligatorio';
      if (!values.email) errors.email = 'El email es obligatorio';
      else if (!emailRegex.test(values.email)) errors.email = 'Formato de email inválido';
